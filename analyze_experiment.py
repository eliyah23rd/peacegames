...
def render_bar_png(values: Dict[str, float], *, title: str) -> bytes:
    ...
    ax.set_title(title)
    ax.set_ylabel("Value")
    ax.tick_params(axis="x", labelsize=16, colors="red")
    ax.tick_params(axis="y", labelsize=14, colors="red")
    ax.set_title(title, fontsize=16, color="red")
    ax.set_ylabel("Value", fontsize=14, color="red")
    ax.set_xticklabels(labels, rotation=25, ha="right", color="red", fontsize=16)
    fig.tight_layout()
    buf = io.BytesIO()
...
def render_box_png(stats: Dict[str, ModifierStats], *, title: str) -> bytes:
    ...
    ax.tick_params(axis="y", labelsize=14, colors="red")
    ax.set_title(title, fontsize=16, color="red")
    ax.set_ylabel("Welfare", fontsize=14, color="red")
    ax.set_xticklabels(labels, rotation=25, ha="right", color="red", fontsize=16)
    fig.tight_layout()
    buf = io.BytesIO()
    fig.savefig(buf, format="png", dpi=150)
